...
import 'dart:math';
import 'dart:async';
import 'package:sneeze/main.dart';
...
class TimePickerSpinner extends StatefulWidget {
  ...
  final TextStyle subnormalTextStyle;
  final double itemHeight;
...
class _TimePickerSpinnerState extends State<TimePickerSpinner> {
  ...
  int currentSelectedHourIndex = -2;
  int currentSelectedMinuteIndex = -2;
  int currentSelectedSecondIndex = -2;
  int currentSelectedAPIndex = -2;
  ...
  TextStyle defaultSubNormalTextStyle = new TextStyle(
      fontSize: 32,
      color: Colors.black54
  );
  ...
  TextStyle _getSubNormalTextStyle(){
    return widget.subnormalTextStyle != null
        ? widget.subnormalTextStyle
        : defaultSubNormalTextStyle;
  }
  ...
  DateTime getDateTime() {
    ...
    print('${hour.toString().length == 1 ? '0$hour' : '$hour'} : ${minute.toString().length == 1 ? '0$minute' : '$minute'}');
      //setState(() {
        timeAbb = '${hour.toString().length == 1 ? '0$hour' : '$hour'}:${minute.toString().length == 1 ? '0$minute' : '$minute'}';
      //});
  ...
  void initState() {
    ...
    hourController = new ScrollController(
        initialScrollOffset: (currentSelectedHourIndex - 2) *
            _getItemHeight());
    ...
    minuteController = new ScrollController(
        initialScrollOffset: (currentSelectedMinuteIndex - 2) *
            _getItemHeight());
    ...
    secondController = new ScrollController(
        initialScrollOffset: (currentSelectedSecondIndex - 2) *
            _getItemHeight());

    currentSelectedAPIndex = currentTime.hour >= 12 ? 2 : 1;
    apController = new ScrollController(
        initialScrollOffset: (currentSelectedAPIndex - 2) * _getItemHeight());
    ...
    if (widget.onTimeChange != null) {
     /* WidgetsBinding.instance
      ...
      WidgetsBinding.instance
          .addPostFrameCallback((_) => getDateTime());
    }
  }
      ...
      new Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.only(
            topRight: Radius.elliptical(1,50),
            topLeft: Radius.elliptical(1,50),
            bottomRight: Radius.elliptical(1,50),
            bottomLeft: Radius.elliptical(1,50),
          ),
          color: Colors.white
        ),
      ),
          ...
          return new Container(
            ...
            child: new Text(
              ...
              style: selectedIndex == index
                  ...
                  : selectedIndex == index - 1 ? _getNormalTextStyle() : selectedIndex == index + 1 ? _getNormalTextStyle() : _getSubNormalTextStyle(),
            ),
          );
        },
          ...
          return new Container(
            ...
            child: new Text(
              ...
              style: currentSelectedAPIndex == index
                  ? _getHighlightedTextStyle()
                  : currentSelectedAPIndex == index - 1
                  ? _getNormalTextStyle(): currentSelectedAPIndex == index + 1
                  ? _getNormalTextStyle() : _getSubNormalTextStyle(),
            ),
          );
        },
